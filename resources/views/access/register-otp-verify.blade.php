...
<div class="card">
    <form method="POST" action="{{ route('register.otp.verify') }}">
        ...
        <p style="background:#F4F8FF; padding:12px; border-radius:12px; font-size:13px; text-align:center">
            <strong>{{ session('register_phone') }}</strong>
        </p>
        ...
        <button type="submit" class="btn gold mt">Verifikasi</button>
    </form>

    <hr style="margin:20px 0; border:none; border-top:1px solid var(--line)">

    <p style="text-align:center; font-size:13px">
        <a href="{{ route('register') }}" style="color:var(--blue); font-weight:700; text-decoration:none">Minta kode baru</a>
    </p>
</div>
@endsection
